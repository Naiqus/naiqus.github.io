...
query_roll_dist_transErr_distErr_rotErr.setQuery("SELECT Pitch, Dist, average(DistErr), average(RotationErr), average(TransformErr) FROM "+tableName+" WHERE Layout = 'Single' AND Roll < 75 GROUP BY Pitch, Dist");
  
    query_roll_pitch_transErr.setQuery("SELECT Roll, Pitch, average(TransformErr) FROM "+tableName+" WHERE Layout = 'Single' GROUP BY Roll, Pitch");
...
function drawChart_dist_transErr(){
  ...
  var chart = new google.visualization.LineChart(document.getElementById('single_dists-transError'));
  var options = {
 ...
 function drawChart_3d_angle_rotationErr()
{
   ...
   var surfacePlot = new greg.ross.visualisation.SurfacePlot(document.getElementById("single_3d_angle_rotationError"));
